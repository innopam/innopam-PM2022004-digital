...
from ..metrics.segmentation import metrics_from_confusion_matrix, multiclass_confusion_matrix
...
class MulticlassChangeDetectionTrainer:
    def __init__(
        self,
        model: nn.Module,
        optimizer: Optimizer,
        ...
        num_classes: int = 3,
        best_metric: str = "foreground_mean_f1",
    ) -> None:
        self.model = model
    ...
    def train_epoch(self, dataloader: Iterable, epoch: int) -> Dict[str, float]:
        ...
        for step, batch in enumerate(dataloader, start=1):
            ...
            with torch.cuda.amp.autocast(enabled=self.autocast_dtype is not None, dtype=self.autocast_dtype):
                logits = self.model(batch["t1"], batch["t2"])
                loss, _ = self._compute_loss(logits, batch["mask"])
    ...
    def validate(self, dataloader: Iterable) -> Dict[str, float]:
        self.model.eval()
        loss_meter = AverageMeter()
        confusion = torch.zeros((self.num_classes, self.num_classes), dtype=torch.float64, device=self.device)
        ...
        for batch in dataloader:
            ...
            confusion += multiclass_confusion_matrix(logits, targets, num_classes=self.num_classes).to(self.device)

        metrics = metrics_from_confusion_matrix(confusion.cpu())
